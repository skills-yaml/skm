...
`workspace-docs@3.0.0` requires spec-driven development for non-trivial work.
...
Legacy spec locations such as `workspace/specs/legacy/` may exist, but active work should use `workspace/specs/`.
...
- implementation plan
- validation gates
- a `Memory Impact` section with `Status: pending`
...
- memory impact is resolved to `updated` or `none` with a rationale
- an `updated` impact identifies the category memory file and `changelog.md`
...
Agents must not declare work complete only because code was changed. Work is
complete when the spec, implementation, validation, documentation, and memory
impact are reconciled. Every final handoff must state `Memory impact: updated`
or `Memory impact: none` with a rationale.
